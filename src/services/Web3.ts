import Web3 from 'web3'
import { Provider } from 'web3/providers'
import Web3ProviderEngine from 'web3-provider-engine'
import Subprovider from 'web3-provider-engine/subproviders/subprovider'
import ProviderSubprovider from 'web3-provider-engine/subproviders/provider'

import { InjectedUserSubprovider } from 'Src/lib'
import config from 'Config'
import log from 'Log'

declare global {
  interface Window {
    ethereum?: any
    web3?: any
    faastWeb3?: any
  }
}

let userSubprovider: Subprovider

const provider = new Web3ProviderEngine()
// Injected user web3 for account methods
provider.addProvider(new InjectedUserSubprovider(() => userSubprovider))
// Public node for lookups
provider.addProvider(new ProviderSubprovider(new Web3.providers.HttpProvider(config.web3Provider)))
provider.on('error', log.error)
provider.start()

class FaastWeb3 extends Web3 {

  enableUserProvider(): Promise<Provider> {
    return Promise.resolve().then(() => {
      if (window.ethereum) {
        // Modern dapp browsers...
        return window.ethereum.enable()
          .then(() => window.ethereum)
      } else if (window.web3) {
        // Legacy dapp browsers...
        return window.web3.currentProvider
      } else {
        throw new Error('No user web3 provider detected')
      }
    }).then((userProvider) => {
      const userWeb3 = new Web3(userProvider)
      if (!userWeb3.eth.net.getId) {
        throw new Error('Unable to determine network ID')
      }
      return userWeb3.eth.net.getId()
        .then((id) => {
          if (id !== 1) {
            throw new Error('Please adjust your wallet to use the Main Ethereum Network')
          }
          userSubprovider = new ProviderSubprovider(userProvider)
          return userProvider
        })
    })
  }
}

const faastWeb3 = new FaastWeb3(provider)
window.faastWeb3 = faastWeb3
export default faastWeb3
